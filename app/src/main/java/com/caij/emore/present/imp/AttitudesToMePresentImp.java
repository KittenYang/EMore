...
public class AttitudesToMePresentImp implements RefreshListPresent {
    ...
    @Override
    public void refresh() {
        Subscription su =  mWeiboSource.getToMeAttiyudes(mToken, 0, 0, Key.WEICO_APP_ID, 1, COUNT)
    ...
    public void loadMore() {
        ...
        Subscription su = mWeiboSource.getToMeAttiyudes(mToken, maxId, 0, Key.WEICO_APP_ID, 1, COUNT)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Subscriber<List<Attitude>>() {
                    @Override
